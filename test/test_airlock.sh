...
if run_pull '\nyes\n' \
	...
	cat "$WORK/out"
	fail "fresh pull did not install"
fi
...
if run_pull 'n\nyes\n' \
	...
	cat "$WORK/out"
	fail "the review-skip path went wrong"
fi
...
if run_pull ''; then
	...
	cat "$WORK/out"
	fail "symlink refusal failed for the wrong reason"
fi
...
if run_pull ''; then
	...
	cat "$WORK/out"
	fail "binary refusal failed for the wrong reason"
fi
...
if run_pull '\nyes\n' && grep -q 'blob.bin' "$WORK/out" && grep -q 'staged:' "$WORK/out"; then
	...
	cat "$WORK/out"
	fail "declared binary handling went wrong"
fi
...
if run_pull ''; then
	...
	cat "$WORK/out"
	fail "file-name refusal failed for the wrong reason"
fi
...
if run_apply 'y\n'; then
	...
	cat "$WORK/out"
	fail "no-plan refusal failed for the wrong reason"
fi
...
run_pull '\nyes\n' || {
	cat "$WORK/out"
	fail "pull of the plan-bearing tree failed"
}
...
if run_apply 'y\ny\ny\n' \
	...
	cat "$WORK/out" "$WORK/qlog" 2>/dev/null
	fail "confirmed apply went wrong"
fi
...
if run_apply 'q\n'; then
	...
	cat "$WORK/out"
	fail "stop behaved unexpectedly"
fi
...
if run_apply 'y\ny\ny\n' 1; then
	...
	cat "$WORK/out"
	fail "failure handling went wrong"
fi
...
run_pull '\nyes\n' || {
	cat "$WORK/out"
	fail "pull of the malformed plan failed"
}
reset_apply
if run_apply ''; then
	...
	cat "$WORK/out"
	fail "malformed-plan refusal failed for the wrong reason"
fi
...
if run_apply 'y\n'; then
	...
	cat "$WORK/out"
	fail "drift refusal failed for the wrong reason"
fi
...
run_pull '\nyes\n' || {
	cat "$WORK/out"
	fail "pull of the template-bearing plan failed"
}
...
if run_apply_t 'y\n' 0 \
	...
	cat "$WORK/out"
	fail "template auto-skip went wrong"
fi
