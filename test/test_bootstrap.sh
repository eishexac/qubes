...
if (cd "$ROOT" && sh bootstrap.sh wgq) >"$out" 2>&1; then
	if grep -q 'airlock pull' "$out" \
		...
		cat "$out"
		fail "bootstrap output is missing pieces"
	fi
	...
	cat "$out"
	fail "bootstrap failed on a healthy tree"
fi
...
if (cd "$ROOT" && sh bootstrap.sh) >"$out" 2>&1; then
	...
	cat "$out"
	fail "no-project refusal failed for the wrong reason"
fi
...
if (cd "$ROOT" && sh bootstrap.sh wgq wgq) >"$out" 2>&1 \
	...
	cat "$out"
	fail "multi-project bootstrap went wrong"
fi
